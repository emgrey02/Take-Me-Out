...
public class FirstPersonOcclusion : MonoBehaviour
{
    [Header("FMOD Event")]
    [SerializeField]
    public FMODUnity.EventReference occEvent;
    ...
    private void Start()
    {
        Audio = RuntimeManager.CreateInstance(occEvent);
        RuntimeManager.AttachInstanceToGameObject(Audio, this.gameObject, GetComponent<Rigidbody>());
        Audio.start();
        Audio.release();

        AudioDes = RuntimeManager.GetEventDescription(occEvent);
